...
class Potenciometro(Node):
    def __init__(self):
        ...
        # Nos suscribimos a los cuatro topicos que publica el ESP32.
        ...
        # Guardamos el ultimo valor que llego de cada topico. Arrancan en 0 y
        # se van actualizando cada vez que entra un mensaje nuevo.
        ...
        # Creamos el archivo CSV con la fecha y hora en el nombre, asi cada
        # corrida queda en su propio archivo y no se pisan.
        filename = f"historial_potenciometro_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        self.csv_file = open(filename, mode='w', newline='')
        self.csv_writer = csv.writer(self.csv_file)
   
        self.csv_writer.writerow(
            ['Tiempo (s)', 'Señal Cruda', 'Posición Cruda',
    ...
    # Cada uno de estos callbacks se llama cuando llega un mensaje a su topico
    # y simplemente guarda el valor nuevo.
    ...
    def log_data(self):
        stamp = self.get_clock().now().to_msg()   # tiempo actual de ROS2
        ...
        self.csv_file.flush()   # forzamos a escribir en disco ya, sin esperar


        self.get_logger().info(
            f'Señal Cruda: {self.current_raw_signal:.2f} V  | '
            f'Pos. Cruda: {self.current_raw_position:.2f}')
        self.get_logger().info(
            f'Señal Acond.:{self.current_conditioned_signal:.2f} V | '
